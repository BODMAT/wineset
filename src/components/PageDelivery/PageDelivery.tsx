import { Instagram } from "../PageHome/Instagram/Instagram";
import { Recommended } from "../Recommended/Recommended";
import { SpecialOffer } from "../SpecialOffer/SpecialOffer";
import { AboutDelivery } from "./AboutDelivery";

export function PageDelivery() {
    return (
        <div className='relative pt-[143px] max-md:pt-[84px]'>
            <SpecialOffer
                imgSrc={`${import.meta.env.BASE_URL}PageDelivery/delivery-bg.webp`}
                supTitle="Delivery Across Ukraine"
                title="Delicacies to Your Doorstep"
                subTitle="VIEW ALL ITEMS"
                subTitleLink="/Cart"
                contentWidth={381}
            />
            <AboutDelivery />
            <Recommended productFilter="champagne" filterByDiscount={false} />
            <SpecialOffer imgSrc="./HomePage/sommelier.jpg"
                supTitle="Sommelier Choice"
                title="NOT SURE WHICH TO CHOOSE? WE ARE READY TO HELP!"
                subTitle="ARTICLES ABOUT WINE"
                subTitleLink="/Articles"
                contentWidth={893} />
            <Instagram />
        </div>
    )
}